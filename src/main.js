...
function setEdit(next){editing=next;if(mainPulse){homes[4].scale.setScalar(1.14);mainPulse=null;}editPulse={start:performance.now(),duration:reduced.matches?0:680};updateLabels();if(editing)scheduleCatalogWarm();announce(editing?'Change mode. Select a block to replace it.':'Change mode closed.');}
