...
window.clockDeviceCanvas=DeviceCanvas.mount(app,{theme:{background:'#526a81',screen:'#4053a8',zoomSurface:'#f2e2cc',zoomButton:'#e97868',zoomText:'#2b2742',zoomButtonText:'#2b2742',zoomBorder:'#2b2742',zoomShadow:'#2b2742',zoomRadius:'8px',zoomButtonRadius:'4px'}});
const cityData=[
...
const storageKey='papel-world-v1';
let slots=[1,2,3,4,0,5,6,7,8];
...
const ink=new THREE.Color('#2b2742'),cream=new THREE.Color('#f2e2cc');
let renderer;
...
function buildHome(){homes.forEach(clearGroup);homes.length=0;$('#clock-targets').replaceChildren();targets.length=0;
 ...
 const b=document.createElement('button');b.className='clock-target';b.dataset.slot=i;b.addEventListener('click',e=>tap(i,e));b.addEventListener('dblclick',()=>openSelector(i,true));$('#clock-targets').append(b);targets.push(b);
 });updateLabels();installHomeControls();}
function updateLabels(){targets.forEach((b,i)=>{b.setAttribute('aria-label',`${editing?'Change':i===4?'Main clock:':'Make main:'} ${cities[slots[i]].name}`);b.setAttribute('aria-pressed',String(i===4));});$('#edit').textContent=editing?'BACK':'CHANGE';app.classList.toggle('editing',editing);}
...
function setEdit(next){editing=next;if(mainPulse){homes[4].scale.setScalar(1.14);mainPulse=null;}editPulse={start:performance.now(),duration:reduced.matches?0:520};updateLabels();if(editing)scheduleCatalogWarm();announce(editing?'Change mode. Tap a clock to replace it.':'Change mode closed.');}
$('#edit').addEventListener('click',()=>{if(!busy)setEdit(!editing);});
...
function openSelector(i,fromDouble=false){if(view!=='home'||viewChange)return;cancelSwap();lastTap=null;if(mainPulse){homes[4].scale.setScalar(1.14);mainPulse=null;}returnToEdit=editing&&!fromDouble;catalogMode=cities[slots[i]].kind==='timer'?'timer':'clocks';if(choices.some(c=>(c.mesh.userData.city.kind??'clocks')!==catalogMode)){choices.forEach(c=>clearGroup(c.mesh));choices=[];}updateModeNav();editPulse=null;selectedSlot=i;busy=true;$('#replace-name').textContent=cities[slots[i]].name;$('#selector').hidden=false;app.classList.add('preparing-selector');$('#search').value='';if(choices.length)centerCatalog(i);else{buildCatalog(true,i);warmCatalogGpu();}viewChange={start:performance.now(),duration:reduced.matches?0:260};announce(`Opening the selector to replace ${cities[slots[i]].name}.`);}
function enterSelector(now){resetHomes();viewChange=null;view='selector';selectorStart=now;selectorClosing=false;window.clockDeviceCanvas.setTheme({screen:'#4053a8'});app.classList.remove('preparing-selector');app.classList.add('choosing');$('#close-selector').focus();announce(`Choose ${catalogMode==='timer'?'a timer':'a city'} to replace ${cities[slots[selectedSlot]].name}.`);}
...
function finishSelectorClose(now=performance.now()){view='home';editing=returnToEdit;idleBlendStart=now;$('#selector').hidden=true;app.classList.remove('choosing','preparing-selector');window.clockDeviceCanvas.setTheme({screen:'#4053a8'});const oldChoices=choices.map(c=>c.mesh);choices=[];selectorClosing=false;homeEntry={start:now,duration:reduced.matches?0:270};busy=true;updateLabels();targets[selectedSlot]?.focus();deferClear(oldChoices);}
...
app.addEventListener('pointermove',event=>{if(view==='home'&&!event.target.closest('.masthead,#mode-nav,.instrument-action,#duration-editor'))host.style.cursor=pickClock(event)?'pointer':'';});
...
function resize(){w=app.clientWidth;h=app.clientHeight;if(!w||!h)return;renderer.setSize(w,h,false);camera.position.z=Math.max(16,16*650/h);camera.aspect=w/h;camera.updateProjectionMatrix();}
...
function stepEditPulse(now){if(!editPulse||transition)return;const p=editPulse.duration?Math.min(1,(now-editPulse.start)/editPulse.duration):1,envelope=Math.sin(Math.PI*p),wobble=Math.sin(Math.PI*p*4)*(1-p),settle=1-smooth(p);homes.forEach((g,i)=>{g.scale.setScalar((i===4?1.14:.80)*(1+envelope*.085));g.rotation.x*=settle;g.rotation.y*=settle;g.rotation.z=(i%2?1:-1)*wobble*.045;anchorAtPrismEnd(g,i);g.userData.face.rotation.x*=settle;g.userData.face.rotation.y*=settle;g.userData.face.rotation.z*=settle;});if(p===1){homes.forEach((g,i)=>{g.position.set(...placements[i]);g.scale.setScalar(i===4?1.14:.80);g.rotation.set(0,0,0);g.userData.face.rotation.set(0,0,0);});editPulse=null;idleBlendStart=now;}}
function stepEditingIdle(now){if(view!=='home'||!editing||busy||editPulse||transition||viewChange||homeEntry||reduced.matches)return;const time=now*.001;homes.forEach((g,i)=>{const phase=i*1.31,orbit=time*2.65+phase,spring=time*6.1+phase*1.37,ax=Math.sin(orbit)*.0062+Math.sin(spring)*.0012,ay=Math.cos(orbit)*.0072+Math.cos(spring)*.0014;const target=new THREE.Quaternion().setFromEuler(new THREE.Euler(ax,ay,Math.sin(time*3.15+phase)*.0035));g.quaternion.slerp(target,smooth((now-idleBlendStart)/280));g.scale.setScalar(i===4?1.14:.80);g.userData.face.rotation.set(-ax*.18,-ay*.18,0);anchorAtPrismEnd(g,i);});}
...
let last=0;
function frame(now){requestAnimationFrame(frame);if(document.hidden||now-last<22)return;const dt=Math.min(40,now-last);last=now;const date=new Date();stepSwap(now);stepEditPulse(now);stepViewChange(now);stepHomeEntry(now);stepEditingIdle(now);stepMainPulse(now);renderer.setScissorTest(false);renderer.setViewport(0,0,w,h);renderer.clear();
 if(view==='home'){homes.forEach((g,i)=>{updateClock(g,date);colorDepth(g,i===4?0:.12);});projectTargets();projectInstrumentControls();renderer.render(scene,camera);}
 else{stepCatalogPan(now,dt);
 const cw=catalog.clientWidth,ch=catalog.clientHeight;catalogCamera.aspect=w/h;catalogCamera.position.set((catalog.scrollLeft+cw/2)/catalogUnit,-(catalog.scrollTop+ch/2)/catalogUnit,h/(2*Math.tan(THREE.MathUtils.degToRad(21))*catalogUnit));catalogCamera.updateProjectionMatrix();
 ...
 let travelY=0;if(catalogTravel){const t=clamp01((now-catalogTravel.start)/360);travelY=-catalogTravel.direction*(1-smoother(t))*h/catalogUnit;if(t===1&&catalogTravel.direction!==0)catalogTravel=null;}choices.forEach(({mesh,x,y})=>{mesh.position.set(x/catalogUnit,-y/catalogUnit+travelY,-(1-p)*15);mesh.scale.setScalar((.34+.66*p)*(1+cushion)*(catalogMode==='chrono'?1.65:1));if(durationContext?.custom&&durationContext.choice?.mesh===mesh){const expansion=smooth((now-durationContext.start)/260);mesh.position.z+=3.5*expansion;mesh.scale.multiplyScalar(1+expansion*.28);}updateClock(mesh,date);colorDepth(mesh,.025);});
...
function updateModeNav(){const index=worldOrder.indexOf(catalogMode);for(const [id,offset,direction]of [['mode-up',2,'up'],['mode-down',1,'down']]){const name=worldOrder[(index+offset)%3];$(`#${id} span`).textContent=worldNames[name];$(`#${id}`).setAttribute('aria-label',`Move ${direction} to ${worldNames[name]}`);}app.classList.toggle('chrono-catalog',catalogMode==='chrono');$('#search-form').hidden=catalogMode==='chrono';$('#search').placeholder=catalogMode==='timer'?'Search duration, e.g. 06:30':'Search a city or country';$('#search').setAttribute('aria-label',catalogMode==='timer'?'Search timer duration':'Search a city or country');$('#search-form label').textContent=catalogMode==='timer'?'FIND A DURATION':'FIND YOUR NEXT TIME ZONE';$('#empty').textContent=catalogMode==='timer'?'No matching timers. Try another duration.':'No matching cities. Try another search.';$('#selector').setAttribute('aria-label',catalogMode==='timer'?'Choose a timer':catalogMode==='chrono'?'Choose a stopwatch':'Choose a city');}
