...
(async()=>{
 ...
 try{
  ...
  const centered=await page.evaluate(()=>{const a=document.querySelector('#catalog').getBoundingClientRect(),b=document.querySelector('[data-city="0"]').getBoundingClientRect();return Math.hypot((a.left+a.width/2)-(b.left+b.width/2),(a.top+a.height/2)-(b.top+b.height/2));});
  ...
  const timerCentered=await page.evaluate(()=>{const a=document.querySelector('#catalog').getBoundingClientRect(),b=document.querySelector('[data-city="34"]').getBoundingClientRect();return Math.hypot((a.left+a.width/2)-(b.left+b.width/2),(a.top+a.height/2)-(b.top+b.height/2));});
  assert(timerCentered<15,`The current timer was not centered (${timerCentered.toFixed(1)} px).`);
  await page.getByRole('searchbox').fill('00:1');await page.waitForTimeout(80);
  assert(await page.locator('#app.infinite-catalog').count()===0,'Timer search results must be finite.');
  console.log('Selector checks passed: wrapped clocks, finite search, occupied-clock swap, and wrapped timers.');
