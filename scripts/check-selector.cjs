...
(async()=>{
 ...
 try{
  ...
  const centerMetrics=id=>page.evaluate(id=>{const el=document.querySelector('#catalog'),choice=document.querySelector(`[data-city="${id}"]`),a=el.getBoundingClientRect(),b=choice.getBoundingClientRect(),catalog={x:a.left+a.width/2,y:a.top+a.height/2},block={x:b.left+b.width/2,y:b.top+b.height/2};return{distance:Math.hypot(catalog.x-block.x,catalog.y-block.y),catalog,block,rect:{top:a.top,height:a.height,bottom:a.bottom},choiceTop:choice.style.top,gridHeight:document.querySelector('#city-grid').style.height,scroll:{left:el.scrollLeft,top:el.scrollTop}};},id);
  const centerDistance=async id=>(await centerMetrics(id)).distance;
  const centered=await centerDistance(0);
  assert(centered<15,`The current clock was not centered (${JSON.stringify(await centerMetrics(0))}).`);
